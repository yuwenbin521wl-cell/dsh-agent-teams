...
/** Read one agent's current context occupancy (prompt tokens / context window), or undefined when unavailable. */
function readMemberContextPressure(ctx: Context, agent: Agent): number | undefined {
  try {
    const proj = (ctx.get('sessionProjection') as
      | { snapshot?: (session: unknown, keys?: string[]) => Record<string, { contextWindow?: number; pressureTokens?: number } | undefined> }
      | undefined)
    if (!proj || typeof proj.snapshot !== 'function') return undefined
    const snap = proj.snapshot(agent.session, ['contextPressure'])
    const cp = snap?.['contextPressure']
    if (!cp || typeof cp.contextWindow !== 'number' || cp.contextWindow <= 0 || typeof cp.pressureTokens !== 'number') return undefined
    return cp.pressureTokens / cp.contextWindow
  } catch { return undefined }
}

/** Bookkeeping-mode assignment: do the work and write a result file the captain reads. */
function bookkeepingAssignmentPrompt(ticket: DispatchTicket, stateDir: string, teamId: string): string {
  const goal = (ticket.teamDescription || '(not provided)').trim()
  const deps = ticket.dependencyOutputs.map(function (d) { return '- ' + d.id + ' ' + d.subject + ': ' + (d.output || '(no output)') }).join('\n')
  return [
    'AgentTeams automatic task assignment (captain bookkeeping mode).',
    '',
    'You are executing as configured member "' + ticket.memberName + '".',
    'Do not call agent_teams_claim_task or agent_teams_update_task; the captain owns task state.',
    'Work only this task, then report by writing a result file.',
    '',
    'Team goal: ' + goal,
    '',
    'Completed dependency results:',
    deps || '(none)',
    '',
    'Task: ' + ticket.taskId + ' - ' + ticket.subject,
    ticket.description || '',
    '',
    ...
    if (located.captainSessionId === agent.id) {
      ...
      if (status === 'idle') {
        ...
        if (occ !== undefined && config.autoReplaceThreshold !== undefined && occ >= config.autoReplaceThreshold) {
          ctx.logger.warn('agent-teams: captain context pressure ' + (occ * 100).toFixed(0) + '% >= ' + (config.autoReplaceThreshold * 100).toFixed(0) + '% threshold; consider handing the team to a fresh session')
        }
      }
      let requeued = false
