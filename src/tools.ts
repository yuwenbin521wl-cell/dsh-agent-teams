...
import type { MemberLlmSelection } from './members.ts'
...
export interface ToolsConfig {
  ...
  maxMembers: number
  /** Auto-replace a member when its context usage crosses this fraction. */
  autoReplaceThreshold?: number
  /** Whether auto-replace is enabled. */
  autoReplaceEnabled?: boolean
  /** When true, captain/plugin performs member task bookkeeping from a result file (works without agent_teams tools on members). */
...
/** Render any tool value as a single text block. */
const textRender = (_args: unknown, value: unknown): Array<{ type: 'text'; text: string }> => [{ type: 'text', text: String(value) }]

/** Enumerate every team directory under a state root. */
async function listTeams(stateRoot: string): Promise<TeamState[]> {
  let names: string[]
  try { names = await readdir(stateRoot) } catch { return [] }
  const out: TeamState[] = []
  for (const name of names) {
    const t = await readTeam(stateRoot, name)
...
export function registerAgentTeamsTools(ctx: Context, config: ToolsConfig): AgentTeamsRuntime {
  ...
  ctx.tools.register(defineTool({
    name: 'agent_teams_list',
    description: 'List every team in this workspace (id, name, phase, member/task counts). Any session can call it, so a fresh captain can discover an existing team to adopt.',
    parameters: {},
    async execute(_args, exec) {
      const caller = requireCaptain(exec)
      ...
      if (teams.length === 0) return 'No teams found under ' + root
      return teams.map(function (t) {
        return t.id + ' | ' + t.name + ' | ' + (t.phase ?? 'running') + ' | members=' + t.members.filter(function (m) { return m.status !== 'removed' }).length + ' tasks=' + t.tasks.length
      }).join('\n')
    },
  ...
  /**
   * After a takeover, spawn fresh members that belong to the NEW captain and
   * hand the old members' unfinished tasks to them. Old members are then marked
   * removed (their tasks/roles are preserved in the records). This makes a new
   * session able to CONTINUE the team's work without being unable to wake the
   * previous captain's members.
   */
  async function rehomeTeamAfterAdopt(caller: Agent, stateRoot: string, teamId: string): Promise<{ rehomed: number; created: string[] }> {
      ...
      for (const old of team.members) {
        ...
        for (const t of team.tasks) {
          if (t.assignee === old.name && (t.status === 'pending' || t.status === 'claimed' || t.status === 'in_progress')) {
            t.assignee = name; t.status = 'pending'; t.attemptId = undefined; t.attempt = (t.attempt ?? 0) + 1; t.reassigning = false; t.updatedAt = Date.now(); rehomed += 1
          }
        }
        old.status = 'removed'
        created.push(name)
  ...
  ctx.tools.register(defineTool({
    name: 'agent_teams_adopt',
    description: 'Adopt an existing team as this session\'s own (reparent captain). Use when the previous captain is unavailable or its context is full. Pass team_id (see agent_teams_list).',
    parameters: { team_id: { type: 'string', required: true, description: 'Team id to adopt.' } },
    async execute(args, exec) {
      const caller = requireCaptain(exec)
      const ws = workspaceOf(caller)
      const root = stateRootOf(ws, config)
      ...
      const adopted = await withTeamLock(teamLockKey(root, teamId), async function () {
        const team = await readTeam(root, teamId)
        if (team === undefined) throw new Error('team ' + teamId + ' not found')
        if (team.members.some(function (m) { return m.id === caller.id && m.status !== 'removed' })) throw new Error('you are already a member of this team; remove yourself first')
        const oldCaptain = team.captainSessionId
        for (const t of team.tasks) {
          if (t.assignee === CAPTAIN_KEY && t.status !== 'completed' && t.status !== 'failed' && t.status !== 'cancelled') {
            t.status = 'pending'; t.assignee = undefined; t.attemptId = undefined; t.reassigning = false; t.updatedAt = Date.now()
          }
        }
        team.captainSessionId = caller.id
        await writeTeam(root, team)
        return { teamId: team.id, oldCaptain: oldCaptain }
      })
...
function memberRuntime(config: ToolsConfig): MemberRuntimeConfig {
  return {
    ...
    bookkeepingByCaptain: config.memberBookkeepingByCaptain ?? false,
  }
}
