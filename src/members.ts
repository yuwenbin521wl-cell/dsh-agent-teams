...
export interface MemberRuntimeConfig {
  ...
  /** Agent presets whose member subagents expose agent_teams_* tools. */
  memberPresets?: string[]
  /** When true, members do not need agent_teams tools (captain closes tasks from result files), so the preset guard is relaxed. */
  bookkeepingByCaptain?: boolean
}
...
export function installMemberSelectionRuntime(_ctx: Context, _stateDir: string): MemberSelectionRuntime {
  // DeepSeek Harness 0.1.2-alpha.4 removed registerContinuableSetup. Member
  // persona, tool filter, model route, and reasoning effort are baked directly
  // into the startContinuable request, so no per-child setup hook is needed.
  // withPending is kept as an API-compatible passthrough.
  return {
    async withPending<T>(
...
export async function spawnMember(
  ...
  // Members inherit the captain's standing agent preset (alpha.4 composeFrom).
  // A minimal/ptc preset composes only its own tool set and strips the runtime
  // agent_teams_* tools, producing members that cannot claim/update tasks.
  // Refuse to spawn under such a preset so the captain switches to a known
  // team-ready preset instead of silently creating a broken worker.
  const presets = ctx.get('agentPresets') as
    | { composedPreset?: (agentCtx: unknown) => string | undefined }
    | undefined
  const preset = presets?.composedPreset?.(captain.ctx)
  const allowList = config.memberPresets ?? ['standard']
  // When captain bookkeeping is on, members need no agent_teams tools (they
  // write result files and the captain closes the task), so a non-standard
  // preset (e.g. liangshen/minimal) is acceptable and must not be blocked.
  if (!(config.bookkeepingByCaptain === true) && preset !== undefined && !allowList.includes(preset)) {
    throw new Error(
      `agent-teams: captain session is on agent preset "${preset}" which does not expose agent_teams tools to member subagents. This session's preset is fixed and cannot be switched here; open a NEW session running the standard preset and create/continue the team there. Allowed presets: [${allowList.join(', ')}].`,
    )
  }
...
export async function deliverToMember(
  ...
  try {
    await ctx.subagents.sendMessage(captain, brandedSessionId(childId), [{ type: 'text', text }], { signal })
    return true
  } catch (error: unknown) {
 ...
 */
export function installRetiredMemberGuard(_ctx: Context, _stateDir: string): void {
  // alpha.4 has no followup / per-child hook to wrap. Retired members are no
  // longer in the scheduler loop, so they cannot be woken or cold-resumed by
  // the team. Nothing to install.
}
