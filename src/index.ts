...
export interface Config {
  ...
  /** Agent presets whose member subagents expose agent_teams_* tools (default [standard]). */
  memberPresets?: string[]
  /** Auto-replace a member when its context usage crosses this fraction (default 0.7). */
  ...
  /** Stop auto-replacing a member after this many replacement generations (default 2); captain/user decides beyond it. */
  maxAutoReplace?: number
...
export const Config: z<Config> = z.object({
  ...
  maxAutoReplace: z.natural().min(1).default(2),
  ...
  // known team-ready preset prevents creating tool-less members.
  memberPresets: z.array(z.string()).default(['standard']),
  // Auto-replace: when a member's context usage crosses this fraction, spawn a
  // fresh member and transfer its open tasks to it.
  autoReplaceThreshold: z.number().min(0).max(1).default(0.7),
  autoReplaceEnabled: z.boolean().default(false),
  memberBookkeepingByCaptain: z.boolean().default(false),
...
4. After approval, the final member configuration is spawned atomically and the scheduler starts ready work. Lead by delegation: monitor with agent_teams_status, send guidance with agent_teams_send_message, and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. If the user requires every member to contribute or report, create one task per required contribution (or message each member directly); never wait for an unassigned member to produce work it was never given. Parallelize: when several independent (no-dependency) tasks are ready and idle members exist, assign each to a DIFFERENT idle member (or leave them unassigned so the scheduler distributes them); do NOT queue all independent work onto a single busy member, or it will serialize. VERIFY every task: after agent_teams_create_task, call agent_teams_status and confirm the task exists, is assigned to a member (or is in the shared pool), and is actually running/dispatched or ready — never tell the user a task is 'created and running' without confirming it. If you create a task but no idle member can take it, assign it to a member or tell the user it is queued waiting; do not leave it dangling/unclaimed.
...
11. Context management: DO NOT compact/compress context to keep working. When a member reaches ~70% context, prefer a fresh member: agent_teams_add_member, or agent_teams_rehome to re-home the context-full member\'s open tasks. When YOUR OWN captain context reaches ~70%, do not compress and do not self-drive; wrap up, then hand the team to a fresh-session captain via agent_teams_adopt (the team state is durable, so re-home members and continue). Before ANY handoff/new-team switch, write the key conclusions/decisions into the relevant task\'s output (agent_teams_update_task output) or a results file, so the new member/captain inherits the true state and nothing is lost. If a decision is awaiting the user (e.g. 甲/乙 choice), record it with agent_teams_set_decision before any handoff so it survives and the next captain re-asks it; clear it with agent_teams_clear_decision after the user decides. Whenever the user states a binding rule in NATURAL LANGUAGE (e.g. 'use v39\'s executor', 'do not self-implement'), proactively call agent_teams_set_contract (mode=append) on your own initiative — do NOT expect the user to type the tool. Append new rules to keep existing ones; restate the active contract at the top of every assignment. Never rely on a rule having been said once in chat. After adopting a team, read <teamDir>/handoff.md and <teamDir>/archive-tasks.json (plus agent_teams_status) before deciding next steps — historical task outputs/results are preserved in the archive even when the slimmed status only shows active tasks; the durable team state IS the continuity; do NOT ask the user to re-explain the previous conversation. Before handing the team to a fresh-session captain (agent_teams_adopt), run agent_teams_cleanup to drain your own subagents so the next takeover starts clean; adopt also auto-cleans orphaned subagents.
...
export function apply(ctx: Context, config: Config): void {
  const resolved: ToolsConfig = {
    ...
    memberPresets: config.memberPresets ?? ['standard'],
    autoReplaceThreshold: config.autoReplaceThreshold ?? 0.7,
    autoReplaceEnabled: config.autoReplaceEnabled ?? false,
    memberBookkeepingByCaptain: config.memberBookkeepingByCaptain ?? false,
